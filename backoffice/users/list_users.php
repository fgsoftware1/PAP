...
<html lang="en" class="h-100">
	...
	<body class="d-flex flex-column h-100">
		...
		<main class="flex-shrink-0">
			<div class="container">
				<h1 class="mt-5">Listagem utilizadores</h1>
				<br>
				</br>
							...
							if($result = executarSQL($query)){
								while($row = mysqli_fetch_array($result)){
									echo "<tr>";
										...
										echo "<td>".$row["Pass"]."</td>";
										echo "<td>
											<a href='./create_user.php?ID=".$row["ID_Utilizador"]."'>
												<button type='button' class='btn btn-warning'>Alterar</button>
											</a>
											<a href='./script_del_user.php?ID=".$row["ID_Utilizador"]."'>
				...
				<nav aria-label="Page navigation example">
					...
					<li class="page-item disabled"><a class="page-link" href="#" tabindex="-1">Previous</a></li>
						<?php
							$total_registos = 0;
							$query1 = "SELECT * FROM utilizadores";
							...
							for($i = 0; $i <= $paginações; $i++){
								if($pagina_atual == $i + 1){
									echo "<li class='page-item active'><a class='page-link' href='list_users.php?pagina=".($i + 1)."'>".($i + 1)."</a></li>";
								}else{
									echo "<li class='page-item'><a class='page-link' href='list_users.php?pagina=".($i + 1)."'>".($i + 1)."</a></li>";
								}
							}
						?>
