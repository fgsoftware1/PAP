...
<html lang="en" class="h-100">
  ...
  ?>
    ...
    <main class="flex-shrink-0">
      <div class="container">
        <h1 class="mt-5">Listagem tipos produtos</h1>
        <br>
        </br>
        <a href="./create_product_type.php">
          <button type="button" class="btn btn-primary">Inserir tipo produto</button>
        </a>
        <br>
        </br>
        <table class="table table-striped">
          <thead>
            <tr>
              <th scope="col">ID_Tipo_produto</th>
              <th scope="col">Nome_Tipo_Produto</th>
              ...
              if($result = executarSQL($query)){
                while($row = mysqli_fetch_array($result)){
                  echo "<tr>";
                    ...
                    echo "<td>
                      <a href='./create_product_type.php?ID=".$row["ID_Tipo_Produto"]."'>
                        <button type='button' class='btn btn-warning'>Alterar</button>
                      </a>
                      <a href='./product_type_del.php?ID=".$row["ID_Tipo_Produto"]."'>
        ...
        <nav aria-label="Page navigation example">
          <ul class="pagination justify-content-end">
						<li class="page-item disabled"><a class="page-link" href="#" tabindex="-1">Previous</a></li>
							<?php
								$total_registos = 0;
								$query1 = "SELECT * FROM tipo_produto";
								...
								for($i = 0; $i <= $paginações; $i++){
									if($pagina_atual == $i + 1){
										echo "<li class='page-item active'><a class='page-link' href='list_product_types.php?pagina=".($i + 1)."'>".($i + 1)."</a></li>";
									}else{
										echo "<li class='page-item'><a class='page-link' href='list_product_types.php?pagina=".($i + 1)."'>".($i + 1)."</a></li>";
									}
								}
							?>
